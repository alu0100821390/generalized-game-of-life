...
{
    if (in_range(i, j)){
        if ((get_cell_status(i, j) == aliveA) || (get_cell_status(i, j) == aliveB) || (get_cell_status(i, j) == aliveC)){
            return true;
        }
    }
    return false;
...
void board_t::update_cell_neighbours (int i, int j)
{
    ...
    for (int a=i-1; a<=i+1; a++){
        for (int b=j-1; b<=j+1; b++){
            if (!((a==i) && (b==j))){
                if((get_cell_status(a, b) == aliveA) || (get_cell_status(a, b) == aliveB) || (get_cell_status(a, b) == aliveC)){
                    alive_neighbours ++;
                }
            }
        }
