...
void manual_input (board_t &game){
	...
	if (n_cells>0){  /* Si se va a introducir alguna célula viva... */
		...
		int n;
		bool dead_cell;
		cout << "\nEnter the amount of alive cells of type A and B." << endl;
		cout << "(The amount of alive cells of type C will be the remaining amount)" << endl;
		cout << "\nAmount of type A: ";
		cin >> amount_a;
		opc_corr(amount_a);
		...
		if ((n_cells-amount_a)>0){
			...
			amount_c = n_cells - (amount_a + amount_b);
			if (amount_c>0){
